...
def blah(item):
    return item[1]
def run():
    resultSet = load_csv('pre-dados/assuntoCompletoPorEstado.csv')
    map(updateCount, resultSet[1:])
    for state,bystateitems in rootRepoByState.items():
        write_to_csv('dados/',state+'.csv', ['assunto','número de processos (2000-2010)' ], bystateitems.items())
    pass
#    *******************
    count = 0
    totalMax = 0
...
def updateCount(countAndSubj):
    depth = 0
    if(countAndSubj[0] != '**' and countAndSubj[0] != '***' and countAndSubj[0] != 'UF'):
